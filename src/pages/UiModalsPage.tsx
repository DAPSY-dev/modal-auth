...
const sections: {
  title: string;
  description: string;
  modals: { state: Exclude<AuthModalState, null>; label: string }[];
}[] = [
  {
    title: 'Logged out',
    description:
      'Use these forms to sign in, register, or request a password reset. Welcome back uses Alex (alex@example.com) as sample display data; submitting it attempts a real login for that address.',
    modals: [
  ...
  {
    title: 'Logged in',
    description: 'You must be logged in to successfully change your password.',
    modals: [{ state: 'changePassword', label: 'Change password' }],
  },
  {
    title: 'Password recovery',
    description:
      'Saving a new password requires a valid recovery session from an email link.',
    modals: [{ state: 'resetPassword', label: 'Reset password' }],
  },
  {
    title: 'Status messages',
    description:
      'Open these messages directly for styling. Opening a success message does not perform its action. Closing the invalid-link modal or requesting another link signs you out.',
    modals: [
  ...
  return (
    <>
      ...
      <main>
        ...
        <p>
          These are the real forms. Submissions can sign you in, create
          accounts, send emails, or change your password.
        </p>
        {sections.map(({ title, description, modals }) => (
      ...
      <AuthModal
        loginForm={
          modal === 'welcomeBack' ? (
            <LoginForm
              key="remembered"
              rememberedUser={{ name: 'Alex', email: 'alex@example.com' }}
              onSwitchAccount={() => dispatch(showModal('login'))}
            />
          ) : (
            <LoginForm key="login" />
          )
      ...
      {dummy === 'frozen' && (
        <AccountFrozenPreview onClose={() => setDummy(null)} />
      )}
    </>
  );
}
