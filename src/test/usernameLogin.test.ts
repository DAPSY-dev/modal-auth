...
const request = (username = 'John_Doe', password = 'password') =>
...
describe('username login endpoint', () => {
  it('authenticates through Supabase and returns only SDK session tokens', async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        Response.json({ allowed: true, email: 'private@example.com' }),
      )
      .mockResolvedValueOnce(
        Response.json({
          access_token: 'access',
    ...
    const response = await createUsernameLoginHandler(
      config,
      fetcher,
    )(request());
  ...
  it.each([null, 'private@example.com'])(
    'does not reveal an email for failed authentication (%s)',
    async (email) => {
      const fetcher = vi
        .fn<typeof fetch>()
        ...
        .mockResolvedValueOnce(
          Response.json(
            { error_code: 'invalid_credentials', msg: 'internal details' },
            { status: 400 },
          ),
        );
      const response = await createUsernameLoginHandler(
  ...
  it('preserves email verification and rate limits', async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        Response.json({ error_code: 'email_not_confirmed' }, { status: 400 }),
      );
