...
const sdk = vi.hoisted(() => ({
  ...
  setSession: vi.fn(), invoke: vi.fn(), rpc: vi.fn(),
}));
vi.mock('../services/supabase', () => ({ isSupabaseConfigured: true, getSupabase: () => ({ auth: sdk, functions: { invoke: sdk.invoke }, rpc: sdk.rpc }) }));
...
it('rejects registration if email confirmation is disabled', async () => {
  ...
  const { authService } = await import('../services/authService');
  await expect(authService.signUp('John', 'john_doe', 'john@example.com', 'password')).rejects.toThrow('Email confirmation must be enabled');
...
it('normalizes username login and gives session tokens only to the Supabase SDK', async () => {
  sdk.invoke.mockResolvedValue({ data: { access_token: 'test-access', refresh_token: 'test-refresh' }, error: null });
  sdk.setSession.mockResolvedValue({ data: { user: verifiedUser }, error: null });
  const { authService } = await import('../services/authService');
  expect(await authService.signIn(' John_Doe ', 'password')).toEqual({ id: '1', email: verifiedUser.email, name: 'John' });
  expect(sdk.invoke).toHaveBeenCalledWith('username-login', { body: { username: 'john_doe', password: 'password' } });
  expect(sdk.setSession).toHaveBeenCalledWith({ access_token: 'test-access', refresh_token: 'test-refresh' });
...
it('keeps direct email login working without the username function', async () => {
  sdk.signInWithPassword.mockResolvedValue({ data: { user: verifiedUser }, error: null });
  const { authService } = await import('../services/authService');
  await authService.signIn(' john@example.com ', 'password');
  expect(sdk.signInWithPassword).toHaveBeenCalledWith({ email: 'john@example.com', password: 'password' });
  expect(sdk.invoke).not.toHaveBeenCalled();
});

it('maps username login rejection without establishing a session', async () => {
  sdk.invoke.mockResolvedValue({ data: null, error: { context: Response.json({ code: 'invalid_credentials' }, { status: 400 }) } });
  const { authService } = await import('../services/authService');
  await expect(authService.signIn('john_doe', 'wrong')).rejects.toEqual({ code: 'invalid_credentials' });
...
it('stores normalized usernames in signup metadata', async () => {
  ...
  const { authService } = await import('../services/authService');
  await authService.signUp('John', ' John_Doe ', 'john@example.com', 'password');
  expect(sdk.rpc).toHaveBeenCalledWith('is_username_available', { requested_username: 'john_doe' });
  expect(sdk.signUp).toHaveBeenCalledWith(expect.objectContaining({ options: expect.objectContaining({ data: { name: 'John', username: 'john_doe' } }) }));
});
...
it('rejects taken usernames before signup and handles a concurrent reservation', async () => {
  ...
  const { authService, UsernameUnavailableError } = await import('../services/authService');
  await expect(authService.signUp('John', 'john_doe', 'john@example.com', 'password')).rejects.toBeInstanceOf(UsernameUnavailableError);
  expect(sdk.signUp).not.toHaveBeenCalled();
  sdk.rpc.mockResolvedValueOnce({ data: true, error: null }).mockResolvedValueOnce({ data: false, error: null });
  sdk.signUp.mockResolvedValue({ data: {}, error: { code: 'unexpected_failure' } });
  await expect(authService.signUp('John', 'john_doe', 'john@example.com', 'password')).rejects.toBeInstanceOf(UsernameUnavailableError);
});
