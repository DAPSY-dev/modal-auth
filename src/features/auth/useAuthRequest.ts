...
export function useAuthRequest() {
  ...
  async function run(action: () => Promise<void>) {
    ...
    setError(null); setLoading(true); dispatch(setBusy(true));
    try { await action(); }
    catch (error) { setError(getAuthError(error)); }
    finally { pending.current = false; setLoading(false); dispatch(setBusy(false)); }
  }
