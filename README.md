...
For styling and demonstrations, visit `/ui` (or use **UI showcase** in the header). The index links to `/ui/modals`, `/ui/buttons`, `/ui/inputs`, and `/ui/icons`. Each component page has a link back to the index. On `/ui/modals`, each modal has its own button, including Welcome Back, all success states, and the invalid-link state. The page uses the same auth store and real forms as the app. Sections explain login and recovery requirements; all modal launchers remain available for styling. Submissions perform real actions, including sending emails and changing passwords. Normal login and sample Welcome back are supplied by the UI page; opening them does not change the remembered identity. The sample uses alex@example.com, and submitting it attempts a real login. Username recovery and Account frozen remain labelled dummy modals.
