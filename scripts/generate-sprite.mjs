...
for (const file of files) {
  ...
  if (!/^[a-z][a-z0-9-]*$/.test(id))
    throw new Error(`Use lowercase kebab-case SVG filenames: ${file}`);
  const dom = new JSDOM(await readFile(new URL(file, source), 'utf8'), {
  ...
  try {
    ...
    for (const element of [svg, ...svg.querySelectorAll('*')]) {
      if (element.hasAttribute('style') || element.localName === 'style') {
        throw new Error(
          `${file}: use SVG presentation attributes instead of embedded CSS`,
        );
      }
    ...
    const ids = new Map(
      [...svg.querySelectorAll('[id]')].map((node) => [
        node.id,
        `icon-${id}-${node.id}`,
      ]),
    );
    for (const element of [svg, ...svg.querySelectorAll('*')]) {
      for (const attribute of [...element.attributes]) {
        let value = attribute.value.replace(/url\(#([^)]*)\)/g, (match, ref) =>
          ids.has(ref) ? `url(#${ids.get(ref)})` : match,
        );
        if (
    ...
    const symbol = dom.window.document.createElementNS(
      'http://www.w3.org/2000/svg',
      'symbol',
    );
    for (const attribute of [...svg.attributes]) {
      if (!['xmlns', 'id'].includes(attribute.name))
...
console.log(
  `Generated ${fileURLToPath(destination)} with ${symbols.length} icons.`,
);
